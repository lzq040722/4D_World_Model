"""Causal inference pipeline for video generation."""
...
class CausalInferencePipeline(torch.nn.Module):
    """Pipeline for causal video generation inference."""
    ...
    def __init__(self, args, device, generator=None, text_encoder=None, vae=None):
        ...
        # Keep KV-cache offsets consistent with the actual latent grid.
        video_height = int(getattr(args, "height", 480))
        video_width = int(getattr(args, "width", 832))
        latent_height = video_height // 8
        latent_width = video_width // 8
        patch_height, patch_width = self.generator.model.patch_size[1:]
        if latent_height % patch_height or latent_width % patch_width:
            raise ValueError(
                f"Video resolution {video_height}x{video_width} is not compatible "
                f"with latent patch size ({patch_height}, {patch_width})"
            )
    ...
    def inference(self, noise: torch.Tensor, text_prompts: List[str],
                  ...
                  return_latents: bool = False, batch_sample=None,
                  profile: bool = False, low_memory: bool = False,
                  skip_decoding: bool = False,
                  structured_noise_sde: Optional[torch.Tensor] = None,
                  device=None) -> torch.Tensor:
        """Perform causal video generation inference."""
